...
from ad_nerf.NeRFs.HeadNeRF.load_audface import load_audface_data
from ad_nerf.NeRFs.HeadNeRF.run_nerf_helpers import *
...
def config_parser():
    ...
    parser.add_argument("--N_rand", type=int, default=512,  # 2048,
...
def train_head(path_config_head):
    parser = config_parser()
    args = parser.parse_args()

    d = {}
    with open(path_config_head) as f:
        for line in f:
            (key, val) = line.split(' = ')
            d[str(key)] = val[:-1]

    args.expname = d['expname']
    args.datadir = d['datadir']
    ...
    if path_config_head is not None:
        f = os.path.join(basedir, expname, 'config.txt')
        with open(f, 'w') as file:
            file.write(open(path_config_head, 'r').read())
